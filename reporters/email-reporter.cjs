...
function extractTestLogs(logs) {
    ...
    if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) return lines.slice(startIdx, endIdx + 1).join('\n');
    return text;
}
...
function extractErrorDetails(logs) {
    ...
    if (failIdx !== -1) return lines.slice(failIdx, Math.min(lines.length, failIdx + 40)).join('\n');
    const errIdx = lines.findIndex(l => l.includes('[ERROR]'));
    if (errIdx !== -1) return lines.slice(errIdx, Math.min(lines.length, errIdx + 20)).join('\n');
    const errLine = lines.findIndex(l => l.includes('Error:') || l.includes('TEST FAILED'));
    if (errLine !== -1) return lines.slice(Math.max(0, errLine - 2), Math.min(lines.length, errLine + 15)).join('\n');
    return null;
}
...
function extractAllWarnings(logs, testTitle = null) {
    ...
    for (const line of lines) {
        ...
        if (line.includes('[DIAG-DEBUG]') && testTitle && !line.includes(testTitle)) {
            continue;
        }
...
function filterTerminalLogsForTest(terminalOutput, testTitle) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('--- Test Started:') && lines[i].includes(testTitle)) {
            testStartIdx = i;
            break;
        }
    }
...
function findVideoForTest(testTitle) {
    ...
    try {
        ...
        const videoFiles = files.filter(f =>
            /\.(webm|mp4|mkv)$/i.test(f) &&
            f.includes(safeName.substring(0, 40))
        );
        if (videoFiles.length > 0) {
            const sorted = videoFiles
                .map(f => ({ file: f, mtime: fs.statSync(path.join(VIDEO_DIR, f)).mtimeMs }))
...
function readDiagnostics(testId, testTitle) {
    ...
    if (testId) {
        ...
        if (fs.existsSync(filePath)) {
            try {
                ...
                console.log(`[REPORTER] Diagnostics found by testId: ${safeId} (warnings=${data.warnings?.length || 0}, skipped=${data.skippedSteps?.length || 0})`);
                return data;
            } catch (e) {
                console.warn(`[REPORTER] Failed to parse diagnostics file: ${filePath}`, e.message);
            }
        }
    }
    ...
    if (testTitle) {
        ...
        if (fs.existsSync(filePath)) {
            try {
                const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
                console.log(`[REPORTER] Diagnostics found by title: ${safeTitle} (warnings=${data.warnings?.length || 0}, skipped=${data.skippedSteps?.length || 0})`);
                return data;
            } catch (e) {}
        }
    }
    ...
    if (testTitle && fs.existsSync(DIAG_DIR)) {
        try {
            ...
            for (const file of files) {
                try {
                    const data = JSON.parse(fs.readFileSync(path.join(DIAG_DIR, file), 'utf-8'));
                    if (data.testcase === testTitle) {
                        console.log(`[REPORTER] Diagnostics found by search: ${file} (warnings=${data.warnings?.length || 0}, skipped=${data.skippedSteps?.length || 0})`);
                        return data;
                    }
                } catch (e) {}
            }
    ...
    console.warn(`[REPORTER] No diagnostics found for testId="${testId}" title="${testTitle}"`);
    return null;
}
...
function formatInfosAsLogs(infos, testTitle = null) {
    ...
    return infos
        ...
        .map(info => {
            const meta = info.meta && Object.keys(info.meta).length > 0
...
function clearDiagnosticsFolder() {
    ...
    try {
        ...
        if (removed > 0) {
            console.log(`🧹 Diagnostics cleared: ${removed} stale file${removed !== 1 ? 's' : ''} from previous run`);
        }
    } catch (err) {
...
function buildTimeBar(time, wallClock, totalDuration, totalTests, workers) {
    const savings = wallClock !== '—' && totalDuration !== '—'
        ? (() => {
...
function buildStatsBar(stats, totalTests) {
    ...
    function statCard(emoji, num, label, pct, color, bgColor, borderColor, delay, active, isFail) {
        const dc = '#94a3b8'; const dbg = '#f1f5f9'; const dbc = '#e2e8f0';
        const c = active ? color : dc; const bg = active ? bgColor : dbg; const bc = active ? borderColor : dbc;
    ...
    const progressBars = [];
    if (pPct > 0) progressBars.push(`<div class="progress-fill" style="height:100%; width:${pPct}%; background:linear-gradient(90deg,#16a34a,#22c55e); float:left;"></div>`);
    if (fPct > 0) progressBars.push(`<div class="progress-fill" style="height:100%; width:${fPct}%; background:linear-gradient(90deg,#dc2626,#ef4444); float:left;"></div>`);
    if (wPct > 0) progressBars.push(`<div class="progress-fill" style="height:100%; width:${wPct}%; background:linear-gradient(90deg,#d97706,#f59e0b); float:left;"></div>`);
    if (sPct > 0) progressBars.push(`<div class="progress-fill" style="height:100%; width:${sPct}%; background:linear-gradient(90deg,#a1a1aa,#d4d4d8); float:left;"></div>`);
...
function buildTestRow(item, addAttachment, index) {
    const { test, logs, logLineCount, errorDetails, allWarnings, allSkippedSteps, videos, images, hasWarning, hasSkippedLogic, isFailure, isPassed, duration } = item;

    let bgCard, borderColor, color, label, emoji;
    if (isFailure)              { emoji = '❌'; bgCard = '#fef2f2'; borderColor = '#fecaca'; color = '#dc2626'; label = 'FAILED'; }
    else if (hasWarning)        { emoji = '⚠️'; bgCard = '#fffbeb'; borderColor = '#fde68a'; color = '#d97706'; label = 'WARNING'; }
    else if (hasSkippedLogic)   { emoji = '⏭️'; bgCard = '#f5f3ff'; borderColor = '#ddd6fe'; color = '#7c3aed'; label = 'SKIPPED'; }
    else                        { emoji = '✅'; bgCard = '#f0fdf4'; borderColor = '#bbf7d0'; color = '#16a34a'; label = 'PASSED'; }

    const rowBg = isFailure ? '#fffbfb' : (hasWarning ? '#fffdf7' : (hasSkippedLogic ? '#faf9ff' : '#ffffff'));
    const delayClass = `delay-${Math.min(index % 6 + 1, 6)}`;

    const attHtml = [];
    if (isPassed && !hasSkippedLogic) {
        attHtml.push(`<span style="color:#d4d4d8; font-size:10px;">— clean —</span>`);
    } else {
        images.forEach(img => {
            if (addAttachment(img)) {
                const n = img.name.length > 18 ? img.name.substring(0, 16) + '..' : img.name;
                attHtml.push(`<span style="display:inline-block; background:#eff6ff; color:#2563eb; padding:2px 6px; border-radius:4px; font-size:7px; margin:1px; font-family:monospace; border:1px solid #bfdbfe;">🖼️${escapeHtml(n)}</span>`);
            }
        });
        videos.forEach(vid => {
            if (addAttachment(vid)) {
                const sz = (fs.statSync(vid.path).size / (1024 * 1024)).toFixed(1);
                attHtml.push(`<span style="display:inline-block; background:#faf5ff; color:#7c3aed; padding:2px 6px; border-radius:4px; font-size:7px; margin:1px; font-family:monospace; border:1px solid #ddd6fe;">🎬 ${sz}MB</span>`);
            } else {
                attHtml.push(`<span style="display:inline-block; background:#fef2f2; color:#dc2626; padding:2px 6px; border-radius:4px; font-size:7px; margin:1px; border:1px solid #fecaca;">⚠️ miss</span>`);
            }
        });
        if (attHtml.length === 0) attHtml.push(`<span style="color:#d4d4d8; font-size:10px;">—</span>`);
    }
    ...
    if (hasSkippedLogic && allSkippedSteps.length > 0) {
        const items = allSkippedSteps.map(s =>
...
class EmailReporter {
    constructor() {
        ...
        this.stats = { passed: 0, failed: 0, skipped: 0, warning_tests: 0, skipped_logic_tests: 0 };
        this.workers = 1;
    }
    ...
    async onEnd() {
        ...
        for (const { test, result } of this.testRuns.values()) {
            ...
            if (diag && diag.browserConsole && diag.browserConsole.length > 0 && (diag.errors?.length > 0 || diag.warnings?.length > 0)) {
                const browserLogs = stripAnsi(diag.browserConsole.join('\n'));
                if (browserLogs) {
                    rawLogs = rawLogs ? rawLogs + '\n\n[BROWSER CONSOLE]\n' + browserLogs : '[BROWSER CONSOLE]\n' + browserLogs;
                }
            }
            ...
            if (result.error) {
                ...
                errorDetails = diag.errors.map(e => {
            ...
            // --- WARNINGS ---
            let allWarnings = diag ? [...new Set(diag.warnings.map(w => w.message))] : [];
            if (allWarnings.length === 0) {
                ...
                if (terminalOut) {
                    ...
                    if (terminalWarnings.length > 0) {
                        ...
                        console.log(`[REPORTER] Using ${terminalWarnings.length} warnings from terminal fallback for: ${test.title}`);
                    }
                }
            }
            ...
            if (allSkippedSteps.length === 0) {
                ...
                if (terminalOut) {
                    ...
                    for (const line of lines) {
                        ...
                        if (test.title && line.includes('(') && !line.includes(`(${test.title})`)) {
                            continue;
                        }
                        const m = line.match(/\[SKIPPED\].*?-\s*(.+)/);
                        if (m) skippedFromTerminal.push({ reason: m[1].trim(), flow: null });
                    }
                    if (skippedFromTerminal.length > 0) {
                        allSkippedSteps = skippedFromTerminal;
                        console.log(`[REPORTER] Using ${skippedFromTerminal.length} skipped steps from terminal fallback for: ${test.title}`);
                    }
                }
            }
            ...
            console.log(`[REPORTER] ${test.title}: failure=${isFailure} hasWarning=${hasWarning}(${allWarnings.length}) hasSkippedLogic=${hasSkippedLogic}(${allSkippedSteps.length}) diag=${diag ? 'found' : 'NULL'}`);
            ...
            let videos = [], images = [];
            if (shouldAttach) {
                images = rawAttachments.filter(a => a.path && /\.(png|jpg|jpeg|gif|webp)$/i.test(a.path));
                videos = rawAttachments.filter(a => a.path && /\.(webm|mp4|mkv)$/i.test(a.path));
            }

            if (shouldAttach && videos.length === 0 && diag && diag.videoPath && fs.existsSync(diag.videoPath)) {
                videos.push({ name: path.basename(diag.videoPath), path: diag.videoPath, contentType: 'video/webm' });
            }

            if (shouldAttach && videos.length === 0) {
                const foundVideo = findVideoForTest(test.title);
                if (foundVideo && fs.existsSync(foundVideo)) {
                    videos.push({ name: path.basename(foundVideo), path: foundVideo, contentType: 'video/webm' });
                }
            }

            allTests.push({
                test, result, logs, logLineCount, errorDetails, allWarnings, allSkippedSteps,
                videos, images,
                hasWarning, hasSkippedLogic, isFailure, isTestSkipped,
        ...
        for (const item of allTests) {
            ...
            const start = item.result.startTime ? new Date(item.result.startTime).getTime() : 0;
            const end = start + item.duration;
            if (start > 0 && start < earliestStart) earliestStart = start;
            if (end > latestEnd) latestEnd = end;
        }
        const wallClockMs = (earliestStart < Infinity && latestEnd > 0) ? latestEnd - earliestStart : totalMs;
        ...
        if (process.env.DAILY_REPORT_EMAILS) {
            ...
            if (this.stats.failed > 0) {
                summaryEmoji = '🔴'; summaryLabel = 'FAILURES DETECTED'; summaryColor = '#dc2626'; summaryBg = '#fef2f2';
            } else if (this.stats.warning_tests > 0) {
                summaryEmoji = '🟡'; summaryLabel = 'WARNINGS FOUND'; summaryColor = '#d97706'; summaryBg = '#fffbeb';
            } else if (this.stats.skipped_logic_tests > 0 || this.stats.skipped > 0) {
                summaryEmoji = '🟣'; summaryLabel = 'SKIPPED STEPS FOUND'; summaryColor = '#7c3aed'; summaryBg = '#f5f3ff';
            } else {
                summaryEmoji = '🟢'; summaryLabel = 'ALL TESTS PASSED'; summaryColor = '#16a34a'; summaryBg = '#f0fdf4';
            }
        ...
        if (process.env.FAILURE_ALERT_EMAILS) {
            ...
            const tableRows = sortedTests.map((item, idx) => buildTestRow(item, addAttachment, idx)).join('');

            let subjectIcon = '✅';
            if (this.stats.failed > 0) subjectIcon = '❌';
            else if (this.stats.warning_tests > 0) subjectIcon = '⚠️';
            else if (this.stats.skipped_logic_tests > 0 || this.stats.skipped > 0) subjectIcon = '⏭️';
