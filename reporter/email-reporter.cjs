...
class EmailReporter {
  ...
  async onEnd() {
    ...
    if (process.env.FAILURE_ALERT_EMAILS?.trim()) {
      ...
      console.log(
        `[REPORTER] Building detailed report (failed=${this.stats.failed}, warnings=${this.stats.warning_tests}, skipped=${this.stats.skipped + this.stats.skipped_logic_tests}, total=${totalTests}, mode=${allClean ? "fast-all-green" : "full-issues"})…`,
      );

      let resultsSectionHtml = "";
      if (allClean) {
        // Fast path: no 70-row table — keeps send time close to daily mail
        resultsSectionHtml = `
                ${buildSectionTitle("✅", "Test Results")}
                <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 12px 12px 12px; width:calc(100% - 24px); border:1px solid #bbf7d0; border-radius:12px; overflow:hidden; background:#f0fdf4;">
                    <tr><td align="center" style="padding:22px 16px; font-family:'Inter','Segoe UI',Arial,sans-serif;">
                        <div style="font-size:28px; margin-bottom:6px;">✅</div>
                        <div style="font-size:14px; font-weight:800; color:#16a34a; letter-spacing:1px;">ALL ${totalTests} TESTS PASSED</div>
                        <div style="font-size:10px; color:#64748b; margin-top:6px;">No failures · no warnings · no skipped steps</div>
                        <div style="font-size:9px; color:#94a3b8; margin-top:4px;">Full per-test log table omitted for speed on clean runs</div>
                    </td></tr>
                </table>`;
      } else {
        const sortedTests = sortTestsByDefinitionOrder(allTests);
        // Only non-clean rows in the heavy table (passed tests summarized above)
        const issueTests = sortedTests.filter(
          (t) =>
            t.isFailure || t.hasWarning || t.hasSkippedLogic || t.isTestSkipped,
        );
        const passedCount = sortedTests.length - issueTests.length;
        ...
        console.log(
          `[REPORTER] Rendering ${issueTests.length} issue rows (+ ${passedCount} passed summarized)…`,
        );
        const tableRows = issueTests
          .map((item, idx) => buildTestRow(item, addAttachment, idx))
          .join("");

        resultsSectionHtml = `
                ${buildSectionTitle("📋", "Test Results — Issues")}
                <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 12px 6px 12px; width:calc(100% - 24px);">
                    <tr><td style="padding:0 0 8px 0; font-family:'Inter','Segoe UI',Arial,sans-serif; font-size:10px; color:#64748b;">
                        ✅ <b style="color:#16a34a;">${passedCount}</b> passed (clean) · showing <b>${issueTests.length}</b> fail/warn/skip below with logs & evidence
                    </td></tr>
                </table>
