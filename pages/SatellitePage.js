...
import { 
  ...
  OUTLINE_WAIT_MS, PREVIEW_WAIT_MS, DETAILS_IMAGE_WAIT_MS, markLogicSkipped
} from '../utils/helpers';
...
export class SatellitePage extends BasePage {
  ...
  async openSatelliteSection() {
    ...
    try {
      ...
      await saveMapScreenshot(this.page, 'satellite', 'section_open_failed', true);
      throw e;
    }
  }
  ...
  async selectProduct(productName) {
    ...
    try {
      if (await productCell.count() > 0) {
        ...
        await saveMapScreenshot(this.page, productName, 'product_not_found', true);
        markLogicSkipped(`Product "${productName}" not found — skipping scene processing`);
        return false;
      }
    } catch (e) {
      addWarning(`Failed to click product "${productName}": ${e.message}`);
      await saveMapScreenshot(this.page, productName, 'product_click_failed', true);
      return false;
    }
  ...
  async waitForScenesTable() {
    ...
    while (Date.now() - startTime < SCENES_LOAD_TIMEOUT) {
      try {
        const processing = this.scenesTableWrapper.locator('.dataTables_processing');
        if (await processing.count() > 0 && await processing.isVisible().catch(() => false)) {
          const elapsed = Math.round((Date.now() - startTime) / 1000);
          if (elapsed !== lastLoggedSecond && elapsed % 10 === 0) {
            logInfo(`Scenes table: DataTables still processing... (${elapsed}s elapsed)`);
        ...
        if (rowCount > 0) {
          ...
          if (!/^No data available/i.test(firstRowText.trim())) {
            ...
            if (await emptyCell.count() === 0) {
              const elapsed = Math.round((Date.now() - startTime) / 1000);
              logInfo(`✅ Scenes table loaded with ${rowCount} row(s) after ${elapsed}s`);
              return 'data';
            }
          }
        }
        ...
        if (await infoEl.count() > 0 && await infoEl.isVisible().catch(() => false)) {
          const infoText = (await getInnerTextSafe(infoEl)).toLowerCase();
          if (/showing|displaying/.test(infoText) && !/0 (entries|scenes|results)/i.test(infoText) && !/no (entries|data|matching)/i.test(infoText)) {
            logInfo(`Scenes table info indicates data loaded: "${infoText}" — waiting for rows to render...`);
            await this.page.waitForTimeout(2000);
            const retryRowCount = await this.scenesRows.count();
            if (retryRowCount > 0) {
              const retryFirstRow = await getInnerTextSafe(this.scenesRows.first());
              if (!/^No data available/i.test(retryFirstRow.trim())) {
                const elapsed = Math.round((Date.now() - startTime) / 1000);
                logInfo(`✅ Scenes table loaded with ${retryRowCount} row(s) after ${elapsed}s`);
                return 'data';
              }
            }
          }
        ...
        if (elapsed !== lastLoggedSecond && elapsed > 0 && elapsed % 15 === 0) {
          logInfo(`Scenes table still loading... (${elapsed}s elapsed, ${rowCount} rows so far)`);
          lastLoggedSecond = elapsed;
        }
    ...
    if (finalRowCount > 0) {
      ...
      if (!/^No data available/i.test(finalFirstRow.trim())) {
        logInfo(`✅ Scenes table loaded with ${finalRowCount} row(s) at the last moment (${totalTime}s)`);
        return 'data';
      }
    }

    logInfo(`Scenes table still empty after ${totalTime}s — no data available for this product/AOI`);
    markLogicSkipped(`Scenes table empty after ${totalTime}s wait — no scenes for this product/AOI`);
    await saveMapScreenshot(this.page, 'scenes', `no_data_after_${totalTime}s`, false);
    return 'empty';
  }
  ...
  _isValidSatelliteImageSrc(src) {
    ...
    if (/marker|icon|pin|static|add-to-cart|show\.png|hide\.png|preview\.png|details\.png/i.test(trimmed)) return false;
  ...
  _doUrlsMatchSceneId(url1, url2, sceneId) {
    ...
    for (const part of dashParts) {
      if (part.length >= 8 && url1.includes(part) && url2.includes(part)) return true;
    }
  ...
  async _findBrowseImage(existingSrcs) {
    try {
      const candidates = this.page.locator('#map img[src*="browse"], #map img[src*="browser"], #map img[src*=".browse"]');
  ...
  async processScene(row, sceneIndex = 0, opts = {}) {
    ...
    const displayText = rowText.length > 80 ? rowText.substring(0, 80) + '...' : rowText;
    ...
    if (sceneId) {
      ...
      logInfo('Scene ID not parsed — proceeding with sceneId-agnostic validation');
    }

    setContext({ flow: 'sceneProcessing', scene: sceneId || `row${sceneIndex}` });
    ...
    if (await outlineBtn.count() > 0) {
      ...
      markLogicSkipped(`Outline button not found for scene ${sceneId || `row${sceneIndex}`}`);
    }
    ...
    if (await previewBtn.count() > 0) {
      ...
      if (clickedPreview) {
        ...
        const previewImgLocator = await this._waitForNewMapImage(imagesBefore, PREVIEW_WAIT_MS);
        
        if (previewImgLocator) {
          const rawSrc = await previewImgLocator.getAttribute('src').catch(() => null);
          ...
          if (this._isValidSatelliteImageSrc(rawSrc)) {
            ...
            await saveMapScreenshot(this.page, idForFile, 'preview_highlighted', false);
            try { await removeAnnotationLabels(this.page); } catch {}
          } else {
            const rejectedSrc = (rawSrc || 'null').substring(0, 80);
            logInfo(`Preview detector found img element but src is invalid: ${rejectedSrc} — trying fallbacks...`);
          }
        }
        ...
        if (!previewValid && sceneId) {
          ...
          if (found) {
            ...
            previewValid = true;
            logInfo(`Preview image found via sceneId fallback: ${previewImageSrc}`);

            try {
              await highlight(this.page, found.locator, { borderColor: 'rgba(0, 200, 120, 0.95)', pause: 1500 });
              await annotateElementLabel(this.page, found.locator, 'PREVIEW ✅');
            } catch {}

            await saveMapScreenshot(this.page, sceneId, 'preview_sceneid_fallback', false);
            try { await removeAnnotationLabels(this.page); } catch {}
          }
        }
        ...
        if (!previewValid) {
          ...
          if (found) {
            previewImageSrc = found.src;
            previewValid = true;
            logInfo(`Preview image found via browse-url fallback: ${previewImageSrc}`);
            ...
            const idForFile = sceneId || `row${sceneIndex + 1}`;
            await saveMapScreenshot(this.page, idForFile, 'preview_browse_fallback', false);
            try { await removeAnnotationLabels(this.page); } catch {}
          }
        }
        ...
        if (previewValid) {
          logInfo(`✅ PASS: Preview image loaded successfully for ${sceneId || displayText}`);
        } else {
          addWarning(`No valid preview image detected on map for ${sceneId || displayText}`);
        ...
        await saveMapScreenshot(this.page, sceneId || `row${sceneIndex + 1}`, 'preview_click_failed', true);
      }
    } else {
      markLogicSkipped(`Preview button not found for scene ${sceneId || `row${sceneIndex}`}`);
    }
        ...
        try {
          ...
          if (this._isValidSatelliteImageSrc(detailSrc)) {
            ...
            if (previewValid && previewImageSrc) {
              ...
              if (previewFile === detailFile) {
              ...
              } else if (this._doUrlsMatchSceneId(previewImageSrc, detailSrc, sceneId)) {
                ...
                logInfo(`INFO: Preview (${previewFile}) and Detail (${detailFile}) show different images — this can be normal.`);
              }
            } else if (!previewValid) {
              logInfo(`✅ PASS: Detail image loaded successfully (preview was not valid, skipping comparison) for ${sceneId || displayText}`);
            } else {
              logInfo(`✅ PASS: Detail image loaded successfully (no preview to compare) for ${sceneId || displayText}`);
            }
          } else {
            addWarning(`Detail modal appeared but image src is not valid for ${sceneId || displayText}`, { 
              src: (detailSrc || '').substring(0, 80) || 'empty' 
            });
            await saveMapScreenshot(this.page, sceneId || `row${sceneIndex + 1}`, 'detail_invalid_image_src', true);
          }

          await modal.locator('button.close, button.btn-danger').first().click();
          await modal.waitFor({ state: 'hidden', timeout: 8000 });
        } catch (e) { 
          addWarning(`Scene detail modal did not appear for ${sceneId || displayText}: ${e.message}`); 
          await saveMapScreenshot(this.page, sceneId || `row${sceneIndex + 1}`, 'detail_modal_error', true);
        }
      } else {
        addWarning(`Details button click failed for ${sceneId || displayText}`);
        await saveMapScreenshot(this.page, sceneId || `row${sceneIndex + 1}`, 'detail_click_failed', true);
      }
    } else {
      markLogicSkipped(`Details button not found for scene ${sceneId || `row${sceneIndex}`}`);
    }
  }
}