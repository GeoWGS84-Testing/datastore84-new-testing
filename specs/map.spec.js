...
test(
  ...
  async ({ page }) => {
    ...
    try {
      ...
      const maskedSearchApiUrl =
        searchApiUrl.replace(
          /([?&]key=)[^&]+/i,
          '$1***'
        );
      ...
      logInfo(
        'Location "Indore" entered successfully'
      );
      ...
      logInfo(
        `Search Autocomplete API URL: ${maskedSearchApiUrl}`
      );

      console.log('\n');

      console.log(
        `Method : ${searchApiResponse.request().method()}`
      );
      ...
      logInfo(
        'Indore location suggestion selected successfully'
      );
      ...
      logInfo(
        'Map moved to the selected location'
      );
      ...
      logInfo(
        'Selected location marker is visible on the map'
      );
      ...
      if (consoleErrors.length) {
        ...
        addWarning(
          'Browser console errors detected during Search flow',
          {
      ...
      if (failedRequests.length) {
        ...
        addWarning(
          'Network request failures detected during Search flow',
          {
      ...
      logInfo(
        'Search Autocomplete API validation completed successfully'
      );

      logInfo(
        'P0 Map Search test completed successfully'
      );
 ...
 test(
  ...
  async ({ page }) => {
    ...
    try {
      ...
      await showStep(
        page,
        'Step 5: Locate and highlight the Locate Me icon'
      );
      ...
      await showStep(
        page,
        'Step 6: Highlight the Locate Me action'
      );
      ...
      logInfo(
        'Geolocation failure simulation enabled'
      );
      ...
      const dialogPromise = new Promise(resolve => {
        ...
        page.once('dialog', async dialog => {
          ...
          logInfo(
            `Browser dialog detected. Type: ${dialogType}`
          );

          logInfo(
            `Browser dialog message: "${dialogMessage}"`
          );
      ...
      await showStep(
        page,
        'Step 8: Verify the Locate Me browser alert'
      );
      ...
      await showStep(
        page,
        'Step 9: Complete the Locate Me alert flow'
      );
      ...
      await showStep(
        page,
        'Step 11: Complete the Locate Me flow'
      );
      ...
      await showStep(
        page,
        'Step 12: Complete the final map state'
      );
      ...
      await showStep(
        page,
        'Step 13: Verify network and console diagnostics'
      );

      if (failedRequests.length === 0) {

        logInfo(
          'No non-ignored network requests failed'
        );
      ...
      logInfo(
        'Locate Me test flow completed successfully'
      );
      ...
      addError(
        `Locate Me test failed: ${error?.message || error}`
      );
...
// ============================================================================
  ...
  async ({ page }) => {
    ...
    try {
      ...
      await showStep(
        page,
        'Step 5: Locate the Upload KML icon'
      );
      ...
      await showStep(
        page,
        'Step 6: Prepare to click the Upload KML icon'
      );
      ...
      await showStep(
        page,
        'Step 8: Close Upload File popup using top-right X'
      );

      await mapPage.highlight(
        mapPage.uploadModalCloseButton,
        {
          borderColor: '#F5A614',
      ...
      await showStep(
        page,
        'Step 12: Locate Choose File control'
      );
      ...
      await showStep(
        page,
        'Step 13: Prepare Choose File control'
      );
      ...
      await showStep(
        page,
        'Step 14: Select downloaded.kml from test data'
      );
      ...
      await showStep(
        page,
        'Step 15: Verify downloaded.kml was selected'
      );

      const selectedFiles =
        await mapPage.fileInput.evaluate(
          input =>
            Array.from(
              input.files || []
            ).map(
      ...
      await showStep(
        page,
        'Step 17: Click Upload and upload the selected KML'
      );
      ...
      await showStep(
        page,
        'Step 18: Wait for the selected KML to be processed'
      );
      ...
      await showStep(
        page,
        'Step 19: Verify Core Services popup/panel opens'
      );

      const coreServicesPanel =
        page.locator('#gw-panel');
      ...
      await showStep(
        page,
        'Step 20: Verify KML File Active status'
      );
      ...
      await showStep(
        page,
        'Step 21: Final KML upload state'
      );
      ...
      await showStep(
        page,
        'Step 23: Locate Choose File control for KMZ upload'
      );
      ...
      await showStep(
        page,
        'Step 24: Select MadhyaPradesh.kmz from test data'
      );
      ...
      await showStep(
        page,
        'Step 25: Verify MadhyaPradesh.kmz was selected'
      );

      const selectedKmzFiles =
        await mapPage.fileInput.evaluate(
          input =>
            Array.from(
              input.files || []
      ...
      await showStep(
        page,
        'Step 26: Locate Upload button for KMZ'
      );
      ...
      await showStep(
        page,
        'Step 27: Click Upload and upload MadhyaPradesh.kmz'
      );
      ...
      const coreServicesPanelAfterKmz =
        page.locator('#gw-panel');
      ...
      await showStep(
        page,
        'Step 36: Review network and browser diagnostics'
      );
      ...
      logInfo(
        'KML and KMZ upload test flow completed successfully'
      );
 ...
 test(
  ...
  async ({ page }) => {
    ...
    try {
      ...
      await showStep(
        page,
        'Step 5: Locate the Coordinates icon'
      );

      await mapPage.highlight(
        mapPage.coordinatesButton,
        {
          borderColor: '#00A6FF',
      ...
      await showStep(
        page,
        'Step 6: Click the Coordinates icon'
      );
      ...
      await showStep(
        page,
        'Step 9: Open Coordinates popup again'
      );
...
try {
  ...
  logInfo(
    'Take Me button clicked successfully'
  );
...
logInfo(
  'Map navigated successfully and marker verification passed'
);
      ...
      await showStep(
        page,
        'Step 17: Locate the AOI draw option'
      );

      await mapPage.highlight(
        mapPage.aoiDrawToolbar,
        {
          borderColor: '#00A6FF',
      ...
      logInfo(
        'Coordinates navigation test flow completed successfully'
      );
      ...
      addError(
        `Coordinates navigation test failed: ${
          error?.message || error
        }`
      );
      ...
      await saveMapScreenshot(
        page,
        'coordinates_navigation_test_failed'
      ).catch(() => {});
 ...
 test(
  ...
  async ({ page }) => {
    ...
    try {
      ...
      await showStep(
        page,
        'Step 5: Locate and highlight the User Guide icon'
      );

      await mapPage.highlight(
        mapPage.userGuideButton,
        {
          borderColor: '#F5A614',
      ...
      await showStep(
        page,
        'Step 6: Click the User Guide icon'
      );

      await mapPage.openUserGuidePopup();

      logInfo(
        'User Guide icon clicked'
      );
      ...
      await showStep(
        page,
        'Step 7: Verify the User Guide tutorial popup opened'
      );
      ...
      await showStep(
        page,
        'Step 8: Highlight the User Guide popup top-right X'
      );

      await mapPage.highlight(
        mapPage.userGuidePopupCloseButton,
        {
          borderColor: '#F5A614',
          label: 'STEP 8: Close X',
          pause: 1000,
      ...
      logInfo(
        'User Guide popup close button highlighted'
      );
      ...
      logInfo(
        'User Guide tutorial popup closed successfully'
      );
      ...
      await showStep(
        page,
        'Step 10: Complete the User Guide flow'
      );
      ...
      logInfo(
        'User Guide popup flow completed successfully'
      );
      ...
      logInfo(
        'User Guide tutorial popup test flow completed successfully'
      );
      ...
      addError(
        `User Guide test failed: ${
          error?.message || error
        }`);

      await saveMapScreenshot(
        page,
        'user_guide_test_failed'
      ).catch(() => {});
 ...
 test(
  ...
  async ({ page }) => {
    ...
    page.on('requestfailed', request => {
      ...
      const isIgnoredAnalyticsRequest =
        ignoredAnalyticsDomains.some(domain =>
          url.includes(domain)
        );

      const isIgnoredGoogleMapTile =
        url.startsWith(
          'https://maps.googleapis.com/maps/vt'
        );

      if (
        isIgnoredAnalyticsRequest ||
        isIgnoredGoogleMapTile
      ) {
        return;
      }
    ...
    try {
      ...
      await showStep(
        page,
        'Step 5: Locate and highlight Map Camera Control'
      );
      ...
      await showStep(
        page,
        'Step 6: Open Map Camera Control'
      );
      ...
      logInfo(
        'Map Camera Control expanded successfully'
      );
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const isDrawVisible = await drawTool
          .isVisible()
          .catch(() => false);
        ...
        if (isDrawVisible) {
          ...
          logInfo(
            `AOI Draw option appeared after ${i} zoom clicks`
          );
      ...
      if (!drawToolFound) {
        throw new Error(
          'AOI Draw option did not appear after zooming'
        );
      }
      ...
      logInfo(
        'Draw Shape option clicked successfully'
      );
      ...
      await showStep(
        page,
        'Step 8: Select the Rectangle AOI draw tool'
      );
      ...
      await showStep(
        page,
        'Step 9: Draw one rectangle AOI on the map'
      );

      const mapBox =
        await mapPage.mapContainer.boundingBox();

      if (!mapBox) {
        throw new Error(
          'Unable to get map bounding box for AOI drawing'
        );
      }
      ...
      logInfo(
        'Core Services popup opened successfully'
      );
      ...
      await showStep(
        page,
        'Step 11: Verify AOI Active status'
      );

      const aoiActiveIndicator =
        page.locator('#gw-aoi-label');
      ...
      logInfo(
        'AOI Active status verified successfully'
      );
      ...
      await showStep(
        page,
        'Step 12: Click World View'
      );

      await mapPage.highlight(
        mapPage.worldViewBtn,
        {
          borderColor: '#6C63FF',
          label: 'World View',
          pause: 1000
      ...
      await showStep(
        page,
        'Step 13: Verify global World View map'
      );
      ...
      logInfo(
        'Global World View action completed successfully'
      );
      ...
      await showStep(
        page,
        'Step 15: Verify previously drawn AOI and area'
      );

      const aoiAreaText =
        page
          .getByText(
            /(?:\d+(?:\.\d+)?)\s*(?:km²|km2|sq\.?\s*km)/i
          )
          .first();
      ...
      await showStep(
        page,
        'Step 17: Click Reset and verify the map is reset'
      );
      ...
      logInfo(
        'Reset completed and map is visible again'
      );
      ...
      if (failedRequests.length > 0) {
        ...
        for (const item of failedRequests) {
          ...
          addWarning(
            `FAILED REQUEST → ${item.failure} | ${item.url}`
          );
        }
      ...
      if (consoleErrors.length > 0) {
        ...
        addWarning(
          `TC-6 console errors captured: ${consoleErrors.length}`
        );

      } else {

        logInfo(
          'TC-6 console diagnostics: No browser console errors detected'
        );
      }
    }
  }
...
test(
  '[P0] 7 - Map Camera Control and AOI Draw: Hand, Circle and Rectangle',
  async ({ page }) => {
    ...
    try {
      ...
      await showStep(
        page,
        'Step 7: Enter Indore and validate Search API'
      );
      ...
      logInfo(
        'Selected location processed successfully'
      );
...
logInfo(
  'Hand tool map movement completed successfully'
      ...
      await showStep(
        page,
        'Step 15: Open AOI Draw Tool again'
      );
      ...
      await showStep(
        page,
        'Step 18: Place Marker on Searched Location'
      );


      const searchedLocationBox =
        await mapContainer.boundingBox();
      ...
      const searchedLocationX =
        searchedLocationBox.x +
        searchedLocationBox.width * 0.50;


      const searchedLocationY =
        searchedLocationBox.y +
        searchedLocationBox.height * 0.50;
      ...
      logInfo(
        'Marker placed on searched location'
      );
      ...
      await showStep(
        page,
        'Step 19: Select Circle AOI tool'
      );
      ...
      const circleCenterX =
        circleMapBox.x +
        circleMapBox.width * 0.50;

      const circleCenterY =
        circleMapBox.y +
        circleMapBox.height * 0.50;
      ...
      await showStep(
        page,
        'Step 21: Verify Circle AOI popup and Active status'
      );
      ...
      const circlePopupCandidates =
        page.getByText(
          /[\d,.]+\s*(?:sq\.?\s*km|km²|km2)/i
        );


      const circlePopupCandidateCount =
        await circlePopupCandidates.count();
      ...
      for (
        let i = 0;
        i < circlePopupCandidateCount;
        i++
      ) {

        const candidate =
          circlePopupCandidates.nth(i);
      ...
      const circlePopupMatch =
        circlePopupText.match(
          circleAreaRegex
        );


      const circleMapMatch =
        circleMapAreaText.match(
          circleAreaRegex
        );
...
await showStep(
  page,
  'Step 23: Select Polygon AOI draw tool'
);
...
const directPolygonCount =
  await directPolygonCandidates.count();

if (directPolygonCount > 0) {

  for (
    let i = 0;
    i < directPolygonCount;
    i++
  ) {

    const candidate =
      directPolygonCandidates.nth(i);
...
if (!polygonDrawButton) {
  ...
  if (
    await shapeButtonCandidates.count() > 0
  ) {

    polygonDrawButton =
      shapeButtonCandidates.first();
  }
}
...
logInfo(
  'Polygon AOI draw tool selected successfully'
);
      ...
      const polygonPointA = {
        x:
          polygonMapBox.x +
          polygonMapBox.width * 0.30,

        y:
          polygonMapBox.y +
          polygonMapBox.height * 0.30
      };
      ...
      const polygonPointB = {
        x:
          polygonMapBox.x +
          polygonMapBox.width * 0.52,

        y:
          polygonMapBox.y +
          polygonMapBox.height * 0.25
      };
      ...
      const polygonPointC = {
        x:
          polygonMapBox.x +
          polygonMapBox.width * 0.68,

        y:
          polygonMapBox.y +
          polygonMapBox.height * 0.42
      };
      ...
      const polygonPointD = {
        x:
          polygonMapBox.x +
          polygonMapBox.width * 0.60,

        y:
          polygonMapBox.y +
          polygonMapBox.height * 0.62
      };
      ...
      const polygonPointE = {
        x:
          polygonMapBox.x +
          polygonMapBox.width * 0.38,

        y:
          polygonMapBox.y +
          polygonMapBox.height * 0.60
      };
      ...
      const polygonCoreServicesPanel =
        page.locator('#gw-panel');
      ...
      const polygonAoiActiveIndicator =
        page.locator('#gw-aoi-label');
      ...
      const polygonMapArea =
        page.locator('#gw-aoi-area');


      const polygonMapAreaText =
        await polygonMapArea.innerText();
      ...
      logInfo(
        `Polygon AOI Active map area: ${polygonMapAreaText}`
      );
      ...
      await showStep(
        page,
        'Step 26: Match Polygon map area with popup area'
      );


      const polygonPopupCandidates =
        page.getByText(
          /[\d,.]+\s*(?:sq\.?\s*km|km²|km2)/i
        );


      const polygonPopupCandidateCount =
        await polygonPopupCandidates.count();
      ...
      for (
        let i = 0;
        i < polygonPopupCandidateCount;
        i++
      ) {

        const candidate =
          polygonPopupCandidates.nth(i);
      ...
      const polygonAreaRegex =
        /([\d,.]+)\s*(?:sq\.?\s*km|km²|km2)/i;


      const polygonPopupMatch =
        polygonPopupText.match(
          polygonAreaRegex
        );


      const polygonMapMatch =
        polygonMapAreaText.match(
          polygonAreaRegex
        );
      ...
      await showStep(
        page,
        'Step 27: Select Rectangle AOI draw tool'
      );
      ...
      await showStep(
        page,
        'Step 28: Draw one Rectangle AOI'
      );


      const rectangleMapBox =
        await mapContainer.boundingBox();
      ...
      const rectStartX =
        rectangleMapBox.x +
        rectangleMapBox.width * 0.25;

      const rectStartY =
        rectangleMapBox.y +
        rectangleMapBox.height * 0.25;

      const rectEndX =
        rectangleMapBox.x +
        rectangleMapBox.width * 0.525;

      const rectEndY =
        rectangleMapBox.y +
        rectangleMapBox.height * 0.525;
      ...
      logInfo(
        'Rectangle AOI drawn successfully on the map'
      );
      ...
      const rectangleCoreServicesPanel =
        page.locator('#gw-panel');
      ...
      const rectangleAoiActiveIndicator =
        page.locator('#gw-aoi-label');
      ...
      const rectangleMapArea =
        page.locator('#gw-aoi-area');


      const rectangleMapAreaText =
        await rectangleMapArea.innerText();
      ...
      logInfo(
        `Rectangle AOI Active map area: ${rectangleMapAreaText}`
      );
      ...
      await showStep(
        page,
        'Step 30: Match Rectangle map area with popup area'
      );


      const rectangleAreaRegex =
        /([\d,.]+)\s*(?:sq\.?\s*km|km²|km2)/i;


      const rectanglePopupCandidates =
        page.getByText(
          /[\d,.]+\s*(?:sq\.?\s*km|km²|km2)/i
        );


      const rectanglePopupCandidateCount =
        await rectanglePopupCandidates.count();
      ...
      for (
        let i = 0;
        i < rectanglePopupCandidateCount;
        i++
      ) {

        const candidate =
          rectanglePopupCandidates.nth(i);
      ...
      const rectanglePopupMatch =
        rectanglePopupText.match(
          rectangleAreaRegex
        );


      const rectangleMapMatch =
        rectangleMapAreaText.match(
          rectangleAreaRegex
        );
      ...
      const rectanglePopupValue =
        Number(
          rectanglePopupMatch[1]
            .replace(/,/g, '')
        );


      const rectangleMapValue =
        Number(
          rectangleMapMatch[1]
            .replace(/,/g, '')
        );
      ...
      if (consoleErrors.length) {
        ...
        addWarning(
          'Browser console errors detected during TC-7',
          {
            errors: consoleErrors
          }
        );
      ...
      if (failedRequests.length) {
        ...
        addWarning(
          'Network request failures detected during TC-7',
          {
            failures: failedRequests
          }
        );
      ...
      logInfo(
        'P0 TC-7 completed successfully'
      );
...
test(
  ...
  async ({ page }) => {
    ...
    try {
      ...
      logInfo(
        'Page loader/logo handling completed'
      );
      ...
      logInfo(
        'Map Camera Control located successfully'
      );
      ...
      await showStep(
        page,
        'Step 7: Click (+) and verify that the map zooms in'
      );
      ...
      expect(
        zoomAfter,
        'Map should still exist after Zoom In'
      ).not.toBeNull();

      logInfo(
        'Zoom (+) clicked successfully'
      );

      logInfo(
        'Map remained visible after Zoom In'
      );
      ...
      await showStep(
        page,
        'Step 8: Click (-) and verify that the map zooms out'
      );
      ...
      logInfo(
        'Map remained visible after Zoom Out'
      );
      ...
      if (
        ...
        logInfo(
          'Right (>) control clicked successfully'
        );

        logInfo(
          'Map moved right successfully'
        );

      } else {

        throw new Error(
          'Map Right (>) camera control was not found'
        );
      ...
      if (
        ...
        logInfo(
          'Left (<) control clicked successfully'
        );

        logInfo(
          'Map moved left successfully'
        );

      } else {

        throw new Error(
          'Map Left (<) camera control was not found'
        );
      ...
      if (
        ...
        logInfo(
          'Up-arrow (^) control clicked successfully'
        );

        logInfo(
          'Map moved down successfully'
        );

      } else {

        throw new Error(
          'Map Down (^) camera control was not found'
        );
      ...
      if (
        ...
        logInfo(
          'Lower arrow control clicked successfully'
        );

        logInfo(
          'Map moved up successfully'
        );

      } else {

        throw new Error(
          'Map Up camera control was not found'
        );
      ...
      logInfo(
        'All Map Camera Controls completed successfully'
      );

      logInfo(
        'TC-8 completed successfully'
      );
      ...
      addError(
        `TC-8 failed: ${error?.message || error}`
      );
      ...
      if (consoleErrors.length > 0) {
        ...
        addWarning(
          `TC-8 console errors captured: ${consoleErrors.length}`
        );
        ...
        logInfo(
          'TC-8 console diagnostics: No browser console errors detected'
        );
  ...
  async ({ page }) => {
    ...
    page.on('response', async (response) => {
      try {
        ...
        addWarning(
          `Unable to process network response: ${
            error?.message || error }`
        );
       }
    });
    ...
    try {
      ...
      try {
        ...
        for (const selector of logoCandidates) {
          ...
          if (await locator.isVisible().catch(() => false)) {
            await highlight(  page,  locator,  'DataStore Logo'
            ).catch(() => {});
      ...
      try {
        ...
        addWarning(
          `Tutorial close handling warning: ${
            error?.message || error
          }`
        );
      }
      ...
      const mapVisible = await mapControl
          .isVisible()
          .catch(() => false);
      ...
      expect( satelliteVisible, 'Satellite control should be visible'  ).toBeTruthy();
      ...
      for (let attempt = 1; attempt <= 10; attempt++) {
        ...
        if (
          satelliteControlVisible || bodyText.toLowerCase().includes('satellite') ) { 
      ...
      satelliteScreenshot = await page.screenshot({  type: 'png',  fullPage: false  });
      ...
      logInfo(
        `Satellite API captured entries: ${satelliteApiResponses.length}` );
      ...
      if (satelliteApiResponses.length === 0) {
        ...
        logInfo(  'No live response captured by listener. Checking browser performance resources...' );

        const resourceUrls = await page.evaluate(() => { return performance
            .getEntriesByType('resource')
            .map((entry) => entry.name);
        });

        const satelliteResource = resourceUrls.find((url) =>  url.includes('/get_tv_satellite_list/')  );

        if (satelliteResource) {
        satelliteApiResponses.push({  url: satelliteResource, status: null, body: null, });

          logInfo( `Satellite API found in performance resources: ${satelliteResource}` );
              }
              }

      expect(
        satelliteApiResponses.length, 'Satellite API /get_tv_satellite_list/ should be requested'  ).toBeGreaterThan(0);

      for (const api of satelliteApiResponses) {  logInfo(
          `SATELLITE API → GET ${api.url}`  );

        if (api.status !== null) {
          logInfo( `SATELLITE API STATUS → ${api.status}`   );

          expect(   api.status, 'Satellite API should return HTTP 200'
          ).toBe(200);  }
        ...
        if (api.body) {
          ...
          try { parsedBody = JSON.parse(api.body);
          } catch {
            addWarning(   'Satellite API response was not valid JSON.' );}

          if (parsedBody) { expect(  parsedBody, 'Satellite API response should exist'  ).toBeTruthy();

            if (
              parsedBody.success !== undefined ) {
              expect(  parsedBody.success,  'Satellite API success should be true'   ).toBeTruthy();  }

            if (
              parsedBody.data !== undefined) {
              expect( Array.isArray(parsedBody.data),  'Satellite API data should be an array').toBeTruthy();
      ...
      expect(
        mapAfterSatelliteVisible, 'Map control should be visible after switching to Satellite').toBeTruthy();
      ...
      if (mapVisualChange) {  logInfo('Visual change detected: Satellite → Map.' );
      ...
      expect(finalMapVisible, 'Map control should be visible after returning from Satellite').toBeTruthy();
      ...
      for (const selector of mapSelectors) { const locator = page
          .locator(selector)
          .first();
        ...
        if (  await locator.isVisible().catch(() => false) ) {
          ...
          logInfo( `Final map found using selector: ${selector}`); break; }}

      expect( finalMapFound, 'Map should be visible after switching back from Satellite' ).toBeTruthy();
      ...
      await saveMapScreenshot( page, 'tc9_map_satellite_toggle_failed' ).catch(() => {}); throw error; } });
