...
test.afterEach(async ({ page }, testInfo) => {
  ...
  const hasIssues =
    errors.length > 0 || warnings.length > 0 || skippedSteps.length > 0;
  const isFailure = testInfo.status !== "passed";
  const isSkipped = testInfo.status === "skipped";

  // ★ DEBUG: Log what we captured
  console.log(
    `[DIAG-DEBUG] testId="${testInfo.testId}" title="${testInfo.title}" warnings=${warnings.length} errors=${errors.length} skipped=${skippedSteps.length} status=${testInfo.status}`,
  );
  ...
  if (artifacts.length > 0) {
    for (const filePath of artifacts) {
      try {
        ...
        console.warn("Failed to attach artifact", filePath);
      }
    }
  }
  ...
  const safeId = String(testInfo.testId)
    .replace(/[:/\\<>?"|*]/g, "_")
    .replace(/\s+/g, "_")
    .replace(/_+/g, "_")
    .substring(0, 150);
  ...
  console.log(
    `[DIAG-DEBUG] File ${exists ? "EXISTS" : "MISSING"}: ${expectedFile}`,
  );
  if (exists) {
    const content = JSON.parse(fs2.readFileSync(expectedFile, "utf-8"));
    console.log(
      `[DIAG-DEBUG] File contains: warnings=${content.warnings?.length || 0} errors=${content.errors?.length || 0} skippedSteps=${content.skippedSteps?.length || 0}`,
    );
  }
