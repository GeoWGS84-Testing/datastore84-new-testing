...
import { clearDiagnostics, showStep, logInfo, addWarning, addError } from '../utils/helpers';

test.use({ trace: 'retain-on-failure', screenshot: 'only-on-failure', video: 'on' });
...
test('[P0] 1 - Web launch: load shell and close tutorial', async ({ page }) => {
  ...
  page.on('response', (response) => {
    ...
    if (url.includes('maps.googleapis.com/maps/api/js')) googleMapsResponses.push(response);
  });
  ...
  if (mapsResponse) {
    expect(mapsResponse.status(), 'Google Maps script status').toBeGreaterThanOrEqual(200);
    expect(mapsResponse.status(), 'Google Maps script status').toBeLessThan(400);
  ...
  const isTelemetry = ({ url }) => url.includes('google-analytics.com') || url.includes('googletagmanager.com');
  const mapsFailures = failedRequests.filter(isMaps);
  const telemetryFailures = failedRequests.filter(isTelemetry);
  const appFailures = failedRequests.filter((failure) => !isMaps(failure) && !isTelemetry(failure));

  if (mapsFailures.length) addWarning('Google Maps network failures detected', { failures: mapsFailures });
  if (telemetryFailures.length) console.warn('[Analytics telemetry failures ignored]', JSON.stringify(telemetryFailures));
  if (appFailures.length) addError('Critical application/vendor network failures detected', { failures: appFailures });
  if (consoleErrors.length) addWarning('Browser console errors detected', { errors: consoleErrors });

  expect(appFailures, 'Critical application/vendor requests should not fail during launch').toEqual([]);
