...
test(
  '[P0] 1 - service popup fuctionality and options ',
  async ({ page }) => {
...
logInfo(
  'Denver, CO, USA location suggestion selected successfully'
      ...
      logInfo(
        'Common Service flow Steps 1 to 10 completed successfully'
      );
...
logInfo(
  'Rectangle Core Services popup is visible'
);
...
logInfo(
  'Rectangle Core Services popup highlighted successfully'
);
...
logInfo(
  'Rectangle AOI Active status is visible'
);
...
logInfo(
  'Rectangle AOI Active status highlighted successfully'
);
...
logInfo(
  'Area of Interest section is available and highlighted'
);
...
await showStep(
  page,
  'Step 19: Verify and click Upload AOI'
);

const uploadAoiButton =
  page.locator(
    '#gw-aoi-tabs > div.gw-aoi-tab'
  ).filter({
    hasText: 'Upload AOI'
  }).first();
...
logInfo(
  'Upload AOI button highlighted successfully'
);
...
logInfo(
  'Upload AOI button clicked successfully'
);
...
await showStep(
  page,
  'Step 20: Upload AOI using KML file'
);

logInfo(
  'Upload AOI → KML upload flow started'
);
...
const uploadFileModal =
  page.locator('#uploadFilesModal').first();
...
logInfo(
  'Upload File popup highlighted successfully'
);
...
const fileInput =
  page.locator(
    '#uploadFilesModal input[type="file"]'
  ).first();
...
logInfo(
  'Choose File control highlighted successfully'
);
...
logInfo(
  'downloaded.kml selected successfully'
);
...
logInfo(
  'KML Upload button highlighted successfully'
);
...
logInfo(
  'KML Upload button clicked successfully'
);
...
logInfo(
  'Waiting for KML upload processing'
);
...
logInfo(
  'Map is visible after KML upload'
);
...
logInfo(
  'AOI Active status is visible after KML upload'
);
...
logInfo(
  'AOI Active status highlighted successfully'
);
...
logInfo(
  'Checking whether Upload File modal is completely closed'
);

const visibleUploadModal =
  page.locator(
...
if (
  await visibleUploadModal.count() > 0
) {

  logInfo(
    'Upload File modal is still active'
  );

  // Try modal close button first
  const modalCloseButton =
    visibleUploadModal.locator(
      '[data-dismiss="modal"], .close, button.close'
    ).first();

  if (
    await modalCloseButton.count() > 0
  ) {
    ...
    logInfo(
      'Upload File modal close button processed'
    );
  }
}
...
if (
  ...
  logInfo(
    'Escape pressed to close remaining Upload File modal'
  );
}
...
logInfo(
  'Upload File modal is completely closed'
);
...
if (
  await modalBackdrop.count() > 0
) {

  logInfo(
    'Modal backdrop is still present, waiting for removal'
  );
...
logInfo(
  'Upload File modal backdrop removed successfully'
);
...
if (
  await kmlAoiArea.count() > 0
) {

  const kmlAoiAreaText =
    (
      await kmlAoiArea.innerText()
    ).trim();
...
await showStep(
  page,
  'Step 21: Verify and click Draw AOI'
);
...
const drawButtonStep21 =
  page.locator(
    '#gw-aoi-tabs > div.gw-aoi-tab[title^="Draw"]'
  ).first();
...
logInfo(
  'Draw AOI button highlighted successfully'
);
...
logInfo(
  'Draw AOI Tool opened successfully'
);
...
const mapContainerStep22 =
  mapPage.mapContainer;

const rectangleMapBoxStep22 =
  await mapContainerStep22.boundingBox();

expect(
  rectangleMapBoxStep22,
  'Map bounding box should be available for Rectangle AOI'
).not.toBeNull();

const rectStartXStep22 =
  rectangleMapBoxStep22.x +
  rectangleMapBoxStep22.width * 0.25;

const rectStartYStep22 =
  rectangleMapBoxStep22.y +
  rectangleMapBoxStep22.height * 0.25;

const rectEndXStep22 =
  rectangleMapBoxStep22.x +
  rectangleMapBoxStep22.width * 0.525;

const rectEndYStep22 =
  rectangleMapBoxStep22.y +
  rectangleMapBoxStep22.height * 0.525;
...
logInfo(
  'Rectangle Service popup is visible'
);
...
logInfo(
  'Rectangle Service popup highlighted successfully'
);
...
logInfo(
  'AOI Active status highlighted successfully'
      ...
      logInfo(
        'P0 Service common flow completed successfully'
      );

    } catch (e) {

      addError(
        'Service common flow failed: ' +
        (e?.message || e)
      );
...
test(
  '[P0] 2 - Satellite Service functionality and filters',
  async ({ page }) => {
...
const serviceOptions =
  serviceGrid.locator('div.gw-svc');

const serviceCount =
  await serviceOptions.count();
...
const resolutionVisible =
  await resolutionSection.isVisible().catch(() => false);
...
const resolutionPanel =
  page
    .locator('.gw-res-slider-container')
    .first();
...
const resolutionMinSlider =
  page
    .locator('#gw-res-slider-min')
    .first();
...
logInfo(
  'Resolution Min control is visible'
);
...
const resolutionMaxSlider =
  page
    .locator('#gw-res-slider-max')
    .first();
...
logInfo(
  'Resolution Max control is visible'
);
...
logInfo(
  'Better than 0.3 meters option is visible'
);
...
logInfo(
  'Better than 0.5 meters option is visible'
);
...
logInfo(
  'Better than 0.8 meters option is visible'
);
...
const satelliteFilters =
  page.locator('#gw-sat-filters').first();
...
const satelliteFilterPanel =
  satelliteFilters.locator('.gw-fb').first();
...
logInfo(
  'Satellite Filter panel is visible'
);
...
const defaultSatellite =
  page.locator('#gw-sat-tags').first();
...
logInfo(
  'Default Satellite section is visible'
);
...
const satelliteTagCount =
  await defaultSatellite.locator('*').count();
...
logInfo(
  `Default satellite verified successfully`
);
...
logInfo(
  'Add Satellite button is visible'
);
...
logInfo(
  'Add Satellite button clicked successfully'
);
...
const satelliteDropdownPanel =
  page.locator('#gw-sat-dropdown-panel').first();
...
logInfo(
  'Add Satellite popup opened successfully'
);
...
await showStep(
  page,
  'Step 12: Verify Date Range and date filter options'
);
...
const fromDate =
  page
    .locator('#gw-date-from')
    .first();

await expect(
  fromDate,
  'From date field should be visible'
).toBeVisible({
  timeout: 10000
});
...
await expect(
  toDate,
  'To date field should be visible'
).toBeVisible({
  timeout: 10000
});
...
logInfo(
  'Last 3 months button is visible'
);
      ...
      await showStep(
        page,
        'Step 13: Verify Cloud Coverage Threshold section'
      );

      const cloudCoverageSection =
        page
          .locator(
            '#gw-panel-body .gw-sec'
          )
          .filter({
      ...
      const cloudCoverageSlider =
        cloudCoverageSection.locator(
          'input[type="range"]'
        ).first();

      if (
        await cloudCoverageSlider.count() > 0
      ) {
        ...
        logInfo(
          'Cloud Coverage Threshold slider verified'
        );
...
const searchImageryText =
  (
    await searchImageryButton.innerText()
  ).trim();
...
logInfo(
  `Search Imagery control verified: ${searchImageryText}`
);
...
logInfo(
  'Satellite results table opened successfully'
);
...
logInfo(
  'Satellite results table verified successfully'
);
...
logInfo(
  'Show entries section is visible'
);
...
logInfo(
  'Show entries dropdown verified and set to 10'
);
...
logInfo(
  `Visible satellite entries after selecting 10: ${visibleRowCount}`
);
...
logInfo(
  'Table Search section is visible'
);
...
logInfo(
  'Satellite table search box verified successfully'
);
...
const headerCells =
  tableHeader.locator('th');

const headerCellCount =
  await headerCells.count();

logInfo(
  `Satellite table contains ${headerCellCount} header columns`
);
...
expect(
  (await buyHeader.innerText()).trim()
).toMatch(/Buy/i);
...
logInfo(
  'Buy column verified successfully'
);
...
expect(
  (await productHeader.innerText()).trim()
).toMatch(/Product/i);
...
logInfo(
  'Product column verified successfully'
);
...
expect(
  (await dateHeader.innerText()).trim()
).toMatch(/Date/i);
...
logInfo(
  'Date column verified successfully'
);
...
const outlineHeaderText =
  (await outlineHeader.innerText()).trim();

const outlineHeaderHtml =
  await outlineHeader.innerHTML();

expect(
  outlineHeaderText.length > 0 ||
  outlineHeaderHtml.length > 0,
...
logInfo(
  'Outline column verified successfully'
);
...
const previewHeaderText =
  (await previewHeader.innerText()).trim();

const previewHeaderHtml =
  await previewHeader.innerHTML();

expect(
  previewHeaderText.length > 0 ||
  previewHeaderHtml.length > 0,
...
logInfo(
  'Preview column verified successfully'
);
...
const metadataHeaderText =
  (await metadataHeader.innerText()).trim();

const metadataHeaderHtml =
  await metadataHeader.innerHTML();

expect(
  metadataHeaderText.length > 0 ||
  metadataHeaderHtml.length > 0,
...
logInfo(
  'Metadata column verified successfully'
);
...
const cancelHeaderText =
  (await cancelHeader.innerText()).trim();

const cancelHeaderHtml =
  await cancelHeader.innerHTML();

expect(
  cancelHeaderText.length > 0 ||
  cancelHeaderHtml.length > 0,
...
logInfo(
  'Cancel column verified successfully'
);
...
logInfo(
  `Showing section verified: ${showingText}`
);
...
logInfo(
  'Previous/Next pagination section is visible'
);
...
logInfo(
  'Previous and Next buttons verified successfully'
);
 // ============================================================
...
if (!nextIsDisabled) {
  ...
  logInfo(
    `After Next click: ${secondPageText}`
  );
  ...
  logInfo(
    'Next button functionality verified successfully'
  );
  ...
  logInfo(
    `After Previous click: ${returnedPageText}`
  );
  ...
  expect(
    ...
    /Showing\s+1\s+to\s+10\s+of\s+\d+\s+entries/i
  );

  logInfo(
    'Previous button functionality verified successfully'
  );
...
logInfo(
  'Step 14 Satellite Results table flow completed successfully'
);
...
logInfo(
  `Outline map content verified: ${outlineContentCount} element(s)`
);

logInfo(
  'Outline is displayed on map successfully'
);
...
const previewCount =
  await previewMapContent.count();

logInfo(
  `Preview image/layer detected on map: ${previewCount} element(s)`
);
...
logInfo(
  'Waiting for Preview image to completely finish loading...'
);
...
logInfo(
  'Preview image/layer completely loaded successfully'
);
...
logInfo(
  'Preview rendering completed and map stabilized'
);
...
logInfo(
  'Preview/AOI image is displayed completely on map'
);
...
logInfo(
  `Preview confirmed ready: ${finalPreviewCount} map element(s)`
);
...
logInfo(
  'Metadata action verified after Preview fully loaded'
);
...
logInfo(
  'Metadata clicked successfully after Preview fully loaded'
);
 // ============================================================
...
logInfo(
  'Scene Detail popup opened successfully'
);
...
logInfo(
  'Scene Detail popup image verified'
);

logInfo(
  `Popup scene image source detected: ${popupImageSrc}`
);
...
logInfo(
  'Parameter and Value columns verified successfully'
);
...
logInfo(
  `Scene Detail contains ${detailRowCount} parameter/value rows`
);
...
const detailShowingText =
  (
    await detailTableInfo.innerText()
  )
    .replace(/\s+/g, ' ')
    .trim();

logInfo(
  `Scene Detail showing section: ${detailShowingText}`
);
...
expect(
  ...
  /Showing\s+1\s+to\s+6\s+of\s+6\s+entries/i
);

logInfo(
  'Showing 1 to 6 of 6 entries verified successfully'
);
...
logInfo(
  'Scene Detail popup closed successfully'
);
...
const mapContentBeforeCancel =
  await mapSceneBeforeCancel.count();
...
const mapContentAfterCancel =
  await mapSceneAfterCancel.count();

logInfo(
  `Map scene/preview content after Cancel: ${mapContentAfterCancel}`
);
...
logInfo(
  'Cancel/X successfully removed the drawn AOI/scene from the map'
);
...
logInfo(
  'Cancel/X functionality verified successfully'
);
...
logInfo(
  'Preview, Outline, Metadata, Scene Detail, Close and Cancel flow verified successfully'
      ...
      logInfo(
        'P0 Satellite Service functionality flow completed successfully'
      );
...
let satelliteTags =
  defaultSatellite.locator('.gw-sat-tag');

let satelliteCount =
  await satelliteTags.count();

logInfo(
  `Currently added satellites: ${satelliteCount}`
);
...
while (true) {
  ...
  const currentSatelliteTags =
    defaultSatellite.locator('.gw-sat-tag');

  const currentCount =
    await currentSatelliteTags.count();
  ...
  logInfo(
    `Satellites remaining before removal: ${currentCount}`
  );
  ...
  const satelliteTag =
    currentSatelliteTags.nth(currentCount - 1);

  const satelliteName =
    (
      await satelliteTag
        .getAttribute('data-sat')
    ) ||
    (
      await satelliteTag.innerText()
    ).trim();

  // ACTUAL REMOVE CONTROL FROM DOM
  const removeButton =
    satelliteTag.locator(
      'span.gw-sat-x'
    );
  ...
  await expect(
    defaultSatellite
      .locator(
        `.gw-sat-tag[data-sat="${satelliteName}"]`
      ),
    `Satellite ${satelliteName} should be removed`
  ).toHaveCount(0);

  logInfo(
    `Removed satellite successfully: ${satelliteName}`
  );
...
const remainingSatelliteTags =
  defaultSatellite.locator(
    '.gw-sat-tag'
  );

const remainingSatelliteCount =
  await remainingSatelliteTags.count();
...
logInfo(
  'All default satellites removed successfully'
);
...
const satelliteOptions =
  satelliteDropdownPanel.locator(
    '[role="option"], ' +
    '.gw-sat-option, ' +
    '.gw-sat-item, ' +
    'label'
...
const satelliteOptionCount =
  await satelliteOptions.count();
...
logInfo(
  `Satellite options available: ${satelliteOptionCount}`
);
...
const firstSatelliteName =
  (
    await firstSatellite.innerText()
  ).trim();

logInfo(
  `First satellite selected from popup: ${firstSatelliteName}`
);
...
logInfo(
  `First satellite clicked: ${firstSatelliteName}`
);
...
const addedSatelliteTag =
  defaultSatellite.locator(
    `.gw-sat-tag[data-sat="${firstSatelliteName}"]`
  );

const addedSatelliteCount =
  await addedSatelliteTag.count();
...
if (addedSatelliteCount === 1) {
  ...
  logInfo(
    `Satellite successfully added: ${firstSatelliteName}`
  );
  ...
  const satelliteTagsAfterAdd =
    defaultSatellite.locator('.gw-sat-tag');

  const satelliteTagsAfterAddCount =
    await satelliteTagsAfterAdd.count();
...
logInfo(
  `Satellite scene rows available: ${actionRowCount}`
);
...
logInfo(
  'First satellite scene row selected successfully'
);
...
await expect(
  outlineCell,
  'Outline action should be visible'
).toBeVisible({
  timeout: 10000
});
...
logInfo(
  `Outline displayed on map: ${outlineContentCount} element(s)`
);
...
const previewCell =
  actionRow.locator('td').nth(4);

await expect(
  previewCell,
  'Preview action should be visible'
).toBeVisible({
  timeout: 10000
});
...
await expect.poll(
  ...
  },
  {
    ...
    intervals: [500, 1000, 1500, 2000],
    message:
      'Preview image/layer should appear after clicking Preview'
  }
).toBeGreaterThan(0);
...
logInfo(
  `Preview image/layer detected: ${previewCount}`
);
...
await expect.poll(
  ...
  },
  {
    ...
    intervals: [500, 1000, 1500, 2000],
    message:
      'Preview image should completely load'
  }
).toBe(true);
...
const popupImageSrc =
  await popupImage.getAttribute('src');
...
logInfo(
  'Scene Detail popup image verified successfully'
);
...
const cancelCell =
  actionRow.locator('td').nth(6);

await expect(
  cancelCell,
  'Cancel/X should remain available'
).toBeVisible({
  timeout: 10000
});
...
logInfo(
  'Cancel/X successfully removed scene/preview from map'
);
